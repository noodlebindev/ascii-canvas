...
- All format and composition-rule markdown files under `formats/` and `composition-rules/`
- Any example artefacts under `examples/`

Out of scope:

- The underlying Claude Code or Anthropic API platforms — please report those
  directly to Anthropic.
- Third-party tools the skill mentions but does not bundle (e.g. graph-easy,
  ditaa, figlet — these are documented as optional manual workflows only).
